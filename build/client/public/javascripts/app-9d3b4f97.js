...
module.exports = UploadQueue = (function() {
  ...
  UploadQueue.prototype.getUploadError = function(items) {
    var size;
...
module.exports = {
  ...
  "modal error updoad size exceed": "For performance reasons, you cannot upload more than %{maxSize} files at the same time. Please try again with fewer files.",
...
module.exports = {
    ...
    "modal error existing folder": "La carpeta \"%{name}\" ya existe. No es posible sobreescribir el nombre de una carpeta.",
    "root folder name": "root",
    ...
    "upload running": "La carga está en curso. No cierre su navegador",
    "modal are you sure": "¿Está usted seguro(a)?",
    "modal delete msg": "Suprimir es definitivo",
    "modal delete ok": "Suprimir",
    "modal cancel": "anular",
    "modal delete error": "%{smart_count} operación de borrar ha fallado, el archivo o la carpeta correspondiente ha sido reintegrado. |||| %{smart_count} operaciones de borrar han fallado, los archivos  y carpetas han sido reintegrados.",
    ...
    "postfix error": "Correo no enviado.\nControle que todas las direcciones de los destinatarios son correctas\ny que su Cozy está bien configurado para enviar mensajes.",
    "yes forgot": "Atrás",
    "no forgot": "Ok",
    "perm": "puede",
    ...
    "change notif": "Chequear este cajón si quiere que l@ notifiquen",
...
module.exports = {
    ...
    "modal error updoad size exceed": "Pour des raisons de performance, vous ne pouvez pas télécharger plus de %{maxSize} fichiers en même temps. Merci de recommencer avec moins de fichiers.",
...
var BACKSPACE_KEY, BaseView, BreadcrumbsView, Errors, File, FilesView, FolderView, Modal, ModalBulkMove, ModalConflict, ModalShareView, UploadStatusView,
  bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
  extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
  hasProp = {}.hasOwnProperty;
...
module.exports = FolderView = (function(superClass) {
  ...
  FolderView.prototype.initialize = function(options) {
    ...
    this.listenTo(this.uploadQueue, 'uploadError', this.onUploadError);
    return this;
  };
  ...
  FolderView.prototype.onFilesSelected = function(event) {
    var _addDirectories, _addFiles, _error, _getUploadError, _saveFile, _success, errors, files, items, ref, ref1, ref2, ref3;
    if (event == null) {
      event = {};
    }
    files = [];
    errors = [];
    _getUploadError = (function(_this) {
      return function() {
        return _this.uploadQueue.getUploadError(files);
      };
    })(this);
    _error = function() {
      var formattedErrors, localeOptions;
      ...
      return new Modal(t('chrome error dragdrop title'), t('chrome error dragdrop content', localeOptions), t('chrome error submit'), null);
    };
    _success = (function(_this) {
      return function() {
        ...
        input = files;
        files = [];
        return _this.uploadQueue.addFolderBlobs(input, _this.model);
      };
    })(this);
    _saveFile = function(entry, path, next) {
      if (path == null) {
        path = '';
      }
    ...
    _addFiles = (function(_this) {
      return function() {
        if (!_this.uploadQueue.getUploadError(files)) {
          return _this.uploadQueue.addBlobs(files, _this.model);
        }
      };
    })(this);
    _addDirectories = function(items, path, callback) {
      var counter, totalEntries;
      if (path == null) {
        path = '';
      }
      totalEntries = items.length;
      counter = 0;
      return async.each(items, function(entry, next) {
        ...
        if (entry.isFile) {
          return _saveFile(entry, path, next);
  ...
  FolderView.prototype.onUploadError = function(event) {
    var data, msg, type;
    if (event == null) {
      event = {};
    }
    type = event.type, data = event.data, msg = event.msg;
    if (data == null) {
      data = {};
    }
    if (msg == null) {
