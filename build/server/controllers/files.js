...
var File, Folder, async, baseController, confirmCanUpload, cozydb, crypto, feed, fs, getFileClass, isStorageError, log, mime, moment, multiparty, normalizePath, pathHelpers, ref, sendBinary, sharing, updateParents;
  ...
  form.on('part', function(part) {
    ...
    keepAlive = function() {
      if (upload) {
        ...
        return updateParents.resetTimeout();
      }
    };
      ...
      return uploadStream = file.attachBinary(part, metadata, function(err) {
        ...
        if (!canceled) {
          ...
          return file.updateAttributes(data, function(err) {
            ...
            File.find(file.id, function(err, file) {
              if (err) {
                log.debug(err);
              }
